...
colors = np.array([['forestgreen', 'firebrick'], ['C0', 'C1']])
scale = 0.8
...
for pmi, pm in enumerate(Pms):
    if compare_modified_HG19:
        plt.plot(R0s, results1[pmi]["FC"], '-', c=colors[pmi, 0], label=r'$H_B = {}, \mathrm{{Pm}} = {}$'.format(HB1, pm))
    if compare_DNS:
...
for pmi, pm in enumerate(Pms):
    if compare_modified_HG19:
        plt.plot(R0s, results2[pmi]["FC"], '-', c=colors[pmi, 1], label=r'$H_B = {}, \mathrm{{Pm}} = {}$'.format(HB2, pm))
    if compare_DNS:
