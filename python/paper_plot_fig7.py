...
for chi, ch in enumerate(chs):
    if compare_modified_HG19:
        ...
        plt.plot(R0s, results[chi]["wf"]**2, '-', c=colors[chi], label=r'Model, $C_2 = {:3.1f}$'.format(1.0 / ch))
if compare_DNS:
    # plt.errorbar(R0s_DNS, FCs_DNS, fmt='X', yerr=FCs_DNS_var, c='firebrick', label=r'DNS')
    plt.plot(R0s_DNS, wfs_DNS**2, 'X', c='firebrick', label=r'DNS')
if log_x:
    plt.xscale("log")
if log_y:
    plt.yscale("log")
...
plt.ylabel(r'$\hat{w}_f^2$')
plt.legend()

plt.tight_layout()
# plt.show()
plt.savefig('figures/Fig7b.pdf')
