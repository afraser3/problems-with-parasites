...
if compare_DNS:  # get results of DNS
    ...
    FCs_DNS = np.zeros((len(HBs), len(R0s_DNS)), dtype=np.float64)
    FCs_DNS_var = np.zeros_like(FCs_DNS)
    for hbi, hb in enumerate(HBs):
        for ri, r0 in enumerate(R0s_DNS):
            FCs_DNS[hbi, ri], FCs_DNS_var[hbi, ri] = OUTfile_reader.get_avg_from_DNS(Pr, r0, hb, Pm, "flux_Chem", with_variance=True)
if compare_hydro_DNS:
    R0s_hydro_DNS = np.array([1.5, 3.0, 5.0, 7.0, 9.0])
    FCs_hydro_DNS = np.zeros((len(R0s_hydro_DNS)), dtype=np.float64)
    FCs_hydro_DNS_var = np.zeros_like(FCs_hydro_DNS)
    for ri, r0 in enumerate(R0s_hydro_DNS):
        # get_avg_from_hydr_DNS(r0, var, with_variance=False)
        FCs_hydro_DNS[ri], FCs_hydro_DNS_var[ri] = OUTfile_reader.get_avg_from_hydr_DNS(r0, "flux_Chem", with_variance=True)  # OUTfile_reader.fluxes_nusselts_wrms_hydr_DNS()[2]
...
scale = 0.8
plt.figure(figsize=(6.4 * scale, 4.8 * scale), constrained_layout=True)
if compare_hydro_DNS:
    plt.errorbar(R0s_hydro_DNS, FCs_hydro_DNS, yerr=FCs_hydro_DNS_var, fmt='.', c='k', label='DNS, $H_B = 0$')
if compare_hydro:
    plt.plot(R0s, results_hydro["FC"], '--', c='grey', label='Parasite model')
    plt.plot(R0s, results_hydro["FC"], '--', c='k')#, label='B13 (hydro)')
for hbi, hb in enumerate(HBs):
    if compare_modified_HG19:
        plt.plot(R0s, results[hbi]["FC"], '-', c='C{}'.format(hbi))#, label=r'$H_B = {}$'.format(hb))
    if compare_DNS:
        # plt.plot(R0s_DNS, FCs_DNS[hbi], 'X', c='C{}'.format(hbi))
        plt.errorbar(R0s_DNS, FCs_DNS[hbi], yerr=FCs_DNS_var[hbi], fmt='X', c='C{}'.format(hbi), label=r'DNS, $H_B = {}$'.format(hb))
    if compare_eq32:
        plt.plot(R0s, results_eq32[hbi]["FC"], '--', c='C{}'.format(hbi))#, label=r'HG19, $H_B = {}$'.format(hb))
if log_x:
    plt.xscale("log")
if log_y:
    plt.yscale("log")
