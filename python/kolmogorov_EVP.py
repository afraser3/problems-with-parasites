...
def gamma_over_k_withTC(delta, wf, HB, DB, Pr, tau, R0, ks, N, Sam=False, get_frequencies=False, test_Sam_no_TC=False, sparse_solve=False, k=3, sparse_matrix=None, Richs_matrix=False):
...
def gammax_kscan_withTC(delta, w, HB, DB, Pr, tau, R0, ks, N, badks_except=False, get_kmax=False, Sam=False, test_Sam_no_TC=False, sparse=False):
    gammas = gamma_over_k_withTC(delta, w, HB, DB, Pr, tau, R0, ks, N, Sam=Sam, test_Sam_no_TC=test_Sam_no_TC, sparse_solve=sparse, sparse_matrix='csr')
    ind = np.argmax(gammas)
    gammax = gammas[ind]
    if badks_except and gammax > 0.0:  # ASSUMING USER DOESN'T CARE ABOUT GAMMA_MAX IF IT'S NEGATIVE
        if gammax == gammas[0]:
